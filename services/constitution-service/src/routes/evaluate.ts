...
router.post('/batch', async (req, res) => {
  try {
    ...
    const evaluationResults = await Promise.all(
      actions.map(action => evaluationService.evaluateAction(action, tenantId))
    );
...
router.get('/logs', async (req, res) => {
  try {
    ...
    const where: any = {};
    if (tenantId) where.tenantId = parseInt(tenantId as string);
    if (minScore !== undefined) where.score = { ...where.score, gte: parseFloat(minScore as string) };
    if (maxScore !== undefined) where.score = { ...where.score, lte: parseFloat(maxScore as string) };
    ...
    const scores = logs.map(log => log.score).filter(score => score !== null) as number[];
  ...
  try {
    ...
    const dailyStats = await prisma.$queryRaw`
      ...
      WHERE created_at >= ${dateThreshold}
        ${tenantId ? `AND tenant_id = ${parseInt(tenantId as string)}` : ''}
