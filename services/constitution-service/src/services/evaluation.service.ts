...
export class EvaluationService {
   ...
   */
  async evaluateAction(action: string, tenantId?: number): Promise<EvaluationResult> {
    ...
    try {
      ...
      const matchedPrinciples: MatchedPrinciple[] = [];
   ...
   */
  private async getRelevantPrinciples(actionEmbedding: number[], tenantId?: number): Promise<any[]> {
   ...
   */
  private async evaluateAgainstPrinciple(action: string, principle: any): Promise<MatchedPrinciple> {
   ...
   */
  private checkForViolations(action: string, principle: string): string[] {
   ...
   */
  private applyCategoryRules(action: string, category: string, baseScore: number): number {
   ...
   */
  private calculateOverallScore(matchedPrinciples: MatchedPrinciple[]): number {
   ...
   */
  private determineCompliance(overallScore: number, violations: Violation[]): 'PASS' | 'FAIL' | 'WARNING' {
   ...
   */
  private determineSeverity(score: number, similarity: number): 'HIGH' | 'MEDIUM' | 'LOW' {
   ...
   */
  private generateSuggestion(principle: string, reasoning: string): string {
    const principleLower = principle.toLowerCase();
   ...
   */
  private generateRecommendations(matchedPrinciples: MatchedPrinciple[], violations: Violation[]): string[] {
